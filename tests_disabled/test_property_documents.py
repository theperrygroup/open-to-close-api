...
from open_to_close.property_documents import PropertyDocumentsAPI
...
def test_list_property_documents_success(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        result = property_documents_api.list_property_documents(
            property_id=property_id, params=params
        )
        mock_method.assert_called_once_with(
            "GET", f"/properties/{property_id}/documents", params=params
        )
    assert result == [{"id": 1, "file_name": "contract.pdf"}]


def test_create_property_document_success(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
    ...
    document_data = {
        ...
        "url": "http://example.com/inspection.pdf",
    }  # Simplified
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_documents_api.create_property_document(
            property_id=property_id, document_data=document_data
        )
        mock_method.assert_called_once_with(
            "POST", f"/properties/{property_id}/documents", json_data=document_data
        )
    assert result == {"id": 2, "file_name": "inspection.pdf"}


def test_retrieve_property_document_success(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
    ...
    document_id = 1
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_documents_api.retrieve_property_document(
            property_id=property_id, document_id=document_id
        )
        mock_method.assert_called_once_with(
            "GET", f"/properties/{property_id}/documents/{document_id}"
        )
    assert result == {"id": 1, "file_name": "contract.pdf"}


def test_update_property_document_success(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
    ...
    mock_response.json.return_value = {
        "data": {"id": 1, "file_name": "contract_v2.pdf"}
    }
    property_id = 200
    document_id = 1
    update_data = {"file_name": "contract_v2.pdf"}
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_documents_api.update_property_document(
            property_id=property_id, document_id=document_id, document_data=update_data
        )
        mock_method.assert_called_once_with(
            "PUT",
...
def test_delete_property_document_success_204(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
    ...
    document_id = 1
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_documents_api.delete_property_document(
            property_id=property_id, document_id=document_id
        )
        mock_method.assert_called_once_with(
            "DELETE", f"/properties/{property_id}/documents/{document_id}"
        )
    assert result == {}


def test_delete_property_document_success_json_response(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_list_property_documents_empty(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        mock_method.assert_called_once_with(
            "GET", f"/properties/{property_id}/documents", params=None
        )
    assert result == []


def test_list_property_documents_no_data_key(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_create_property_document_no_data_key(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_retrieve_property_document_no_data_key(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_property_document_no_data_key(
    property_documents_api: PropertyDocumentsAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_documents_api.update_property_document(
            property_id, document_id, update_data
        )
        mock_method.assert_called_once_with(
            "PUT",
            f"/properties/{property_id}/documents/{document_id}",
