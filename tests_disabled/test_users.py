...
from open_to_close.users import UsersAPI
...
def test_update_user_success(users_api: UsersAPI, client: OpenToCloseAPI) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        mock_method.assert_called_once_with(
            "PUT", f"/users/{user_id}", json_data=update_data
        )
...
def test_delete_user_success_json_response(
    users_api: UsersAPI, client: OpenToCloseAPI
) -> None:
