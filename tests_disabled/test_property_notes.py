...
from open_to_close.property_notes import PropertyNotesAPI
...
def test_list_property_notes_success(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        result = property_notes_api.list_property_notes(
...
def test_create_property_note_success(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
    ...
    mock_response.json.return_value = {
        "data": {"id": 2, "content": "Schedule showing."}
    }
    property_id = 500
    note_data = {"content": "Schedule showing.", "is_important": True}
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_notes_api.create_property_note(
            property_id=property_id, note_data=note_data
        )
        mock_method.assert_called_once_with(
...
def test_retrieve_property_note_success(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
    ...
    note_id = 1
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_notes_api.retrieve_property_note(
...
def test_update_property_note_success(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "data": {"id": 1, "content": "Plumber called - appointment set."}
    }
    property_id = 500
    note_id = 1
    update_data = {"content": "Plumber called - appointment set."}
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_notes_api.update_property_note(
            property_id=property_id, note_id=note_id, note_data=update_data
        )
        mock_method.assert_called_once_with(
            "PUT", f"/properties/{property_id}/notes/{note_id}", json_data=update_data
        )
    assert result == {"id": 1, "content": "Plumber called - appointment set."}


def test_delete_property_note_success_204(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
    ...
    note_id = 1
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_notes_api.delete_property_note(
            property_id=property_id, note_id=note_id
        )
        mock_method.assert_called_once_with(
...
def test_delete_property_note_success_json_response(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
...
def test_list_property_notes_empty(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        mock_method.assert_called_once_with(
            "GET", f"/properties/{property_id}/notes", params=None
        )
    assert result == []


def test_list_property_notes_no_data_key(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
...
def test_create_property_note_no_data_key(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
...
def test_retrieve_property_note_no_data_key(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_property_note_no_data_key(
    property_notes_api: PropertyNotesAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_notes_api.update_property_note(
            property_id, note_id, update_data
        )
        mock_method.assert_called_once_with(
            "PUT", f"/properties/{property_id}/notes/{note_id}", json_data=update_data
        )
