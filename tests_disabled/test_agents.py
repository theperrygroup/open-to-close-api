...
def test_update_agent_success(agents_api: AgentsAPI, client: OpenToCloseAPI) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        mock_method.assert_called_once_with(
            "PUT", f"/agents/{agent_id}", json_data=update_data
        )
    assert result == {"id": 1, "name": "Updated Agent"}


def test_delete_agent_success_204(
    agents_api: AgentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_delete_agent_success_json_response(
    agents_api: AgentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_create_agent_no_data_key(
    agents_api: AgentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_retrieve_agent_no_data_key(
    agents_api: AgentsAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_agent_no_data_key(
    agents_api: AgentsAPI, client: OpenToCloseAPI
) -> None:
