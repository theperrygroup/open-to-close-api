...
def test_list_contacts_success(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
...
def test_retrieve_contact_success(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_contact_success(
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = contacts_api.update_contact(
            contact_id=contact_id, contact_data=update_data
        )
        mock_method.assert_called_once_with(
            "PUT", f"/contacts/{contact_id}", json_data=update_data
        )
    assert result == {"id": 1, "name": "Johnathan Doe"}


def test_delete_contact_success_204(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
...
def test_delete_contact_success_json_response(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
...
def test_list_contacts_no_data_key(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
...
def test_create_contact_no_data_key(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
...
def test_retrieve_contact_no_data_key(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_contact_no_data_key(
    contacts_api: ContactsAPI, client: OpenToCloseAPI
) -> None:
