...
from open_to_close.property_tasks import PropertyTasksAPI
...
def test_list_property_tasks_success(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        result = property_tasks_api.list_property_tasks(
            property_id=property_id, params=params
        )
        mock_method.assert_called_once_with(
            "GET", f"/properties/{property_id}/tasks", params=params
        )
    assert result == [{"id": 1, "title": "Order appraisal"}]


def test_create_property_task_success(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
    ...
    mock_response.json.return_value = {
        "data": {"id": 2, "title": "Schedule inspection"}
    }
    property_id = 600
    task_data = {"title": "Schedule inspection", "due_date": "2024-12-31"}
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_tasks_api.create_property_task(
            property_id=property_id, task_data=task_data
        )
        mock_method.assert_called_once_with(
            "POST", f"/properties/{property_id}/tasks", json_data=task_data
        )
    assert result == {"id": 2, "title": "Schedule inspection"}


def test_retrieve_property_task_success(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
    ...
    task_id = 1
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_tasks_api.retrieve_property_task(
            property_id=property_id, task_id=task_id
        )
        mock_method.assert_called_once_with(
            "GET", f"/properties/{property_id}/tasks/{task_id}"
        )
    assert result == {"id": 1, "title": "Order appraisal"}


def test_update_property_task_success(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "data": {"id": 1, "title": "Order appraisal - URGENT"}
    }
    property_id = 600
    task_id = 1
    update_data = {"title": "Order appraisal - URGENT"}
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_tasks_api.update_property_task(
            property_id=property_id, task_id=task_id, task_data=update_data
        )
        mock_method.assert_called_once_with(
            "PUT", f"/properties/{property_id}/tasks/{task_id}", json_data=update_data
        )
    assert result == {"id": 1, "title": "Order appraisal - URGENT"}


def test_delete_property_task_success_204(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
    ...
    task_id = 1
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_tasks_api.delete_property_task(
            property_id=property_id, task_id=task_id
        )
        mock_method.assert_called_once_with(
            "DELETE", f"/properties/{property_id}/tasks/{task_id}"
        )
    assert result == {}


def test_delete_property_task_success_json_response(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
...
def test_list_property_tasks_empty(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        mock_method.assert_called_once_with(
            "GET", f"/properties/{property_id}/tasks", params=None
        )
    assert result == []


def test_list_property_tasks_no_data_key(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
...
def test_create_property_task_no_data_key(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
...
def test_retrieve_property_task_no_data_key(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_property_task_no_data_key(
    property_tasks_api: PropertyTasksAPI, client: OpenToCloseAPI
) -> None:
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = property_tasks_api.update_property_task(
            property_id, task_id, update_data
        )
        mock_method.assert_called_once_with(
            "PUT", f"/properties/{property_id}/tasks/{task_id}", json_data=update_data
        )
