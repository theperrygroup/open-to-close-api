...
from open_to_close.properties import PropertiesAPI
...
def test_list_properties_success(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
...
def test_create_property_success(
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        ...
        mock_method.assert_called_once_with(
            "POST", "/properties", json_data=property_data
        )
    assert result == {"id": 2, "address": "456 Oak Ave"}


def test_retrieve_property_success(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_property_success(
    ...
    with patch.object(client, "_request", return_value=mock_response) as mock_method:
        result = properties_api.update_property(
            property_id=property_id, property_data=update_data
        )
        mock_method.assert_called_once_with(
            "PUT", f"/properties/{property_id}", json_data=update_data
        )
    assert result == {"id": 1, "address": "123 Main Street"}


def test_delete_property_success_204(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
...
def test_delete_property_success_json_response(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
...
def test_list_properties_no_data_key(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
...
def test_create_property_no_data_key(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
...
def test_retrieve_property_no_data_key(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
...
def test_update_property_no_data_key(
    properties_api: PropertiesAPI, client: OpenToCloseAPI
) -> None:
